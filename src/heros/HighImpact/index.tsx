...
type HighImpactHeroProps = Page['hero'] & {
  /**
   * When true (homepage), the hero renders hardcoded marketing copy in a single
   * centered column and hides the eligibility form. When false/undefined (inner
   * pages), it renders CMS `richText` in the original two-column layout with the
...
export const HighImpactHero: React.FC<HighImpactHeroProps> = ({ media, richText, staticContent }) => {
  ...
  const ctaButtons = (
    <AnimatedGroup
      preset="slide"
      className={`flex w-full flex-col items-center justify-center gap-2 sm:gap-2 sm:flex-row sm:items-start ${
        staticContent ? 'lg:justify-center' : 'lg:justify-start'
      }`}
    >
      <Button
        asChild
  ...
  return (
    <main>
      <section className="relative overflow-hidden">
        ...
        {staticContent ? (
          /* Homepage — hardcoded copy, single centered column, no eligibility form */
          ...
          /* Inner pages — CMS-driven content, original two-column layout */
